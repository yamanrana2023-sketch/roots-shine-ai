    ...
    <nav className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${scrolled ? "bg-background/95 backdrop-blur-md border-b border-border/50 shadow-sm" : "bg-transparent"}`}>
        ...
        <div className="hidden md:flex items-center gap-8">
          {links.map((l) => (
            <a
              key={l.href}
              href={l.href}
              className={`text-sm font-medium transition-colors duration-200 ${scrolled ? "text-muted-foreground hover:text-primary" : "text-primary-foreground/70 hover:text-primary-foreground"}`}
            >
              {l.label}
            </a>
          ))}
      ...
      {open && (
        <div className="md:hidden bg-background border-t border-border px-4 pb-4 shadow-lg animate-reveal-up">
          ...
          <a
            href={`tel:${content.contactPhone}`}
            className="inline-flex items-center gap-2 mt-2 bg-accent text-accent-foreground px-5 py-2.5 rounded-xl text-sm font-semibold"
          >
            <Phone className="h-4 w-4" />
            Call Now
          </a>
